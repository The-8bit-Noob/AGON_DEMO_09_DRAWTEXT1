  ...
  320  SC=ABS(SC+0.4) : REM INCREASE SCALE X 0.4 EACH LOOP.
  330  VDU 16 : REM CLEAR GRAPHICS EACH LOOP.
  340  AX%=AX%-10 : AY%=DAY% : REM MOVE A's X POSITION LEFT 10 PIXELS EACH LOOP (keep the text central).
  350  GX%=AX%+16*SC : GY%=DAY% : REM VARS FOR G X & Y (16 pixels + scaling after A).
  360  OX%=GX%+16*SC : OY%=DAY% : REM VARS FOR for O X & Y (16 pixels + scaling after G).
  370  NX%=OX%+16*SC : NY%=DAY% : REM VARS FOR N X & Y (16 pixels + scaling after O).
  400  PROC_DRAW_A
  410  PROC_DRAW_G
  420  PROC_DRAW_O
  430  PROC_DRAW_N
